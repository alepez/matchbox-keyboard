...
static void mb_kbd_xembed_set_win_info(MBKeyboardUI* ui, int flags) {
  CARD32 list[2];

  Atom atom_ATOM_XEMBED_INFO;

  atom_ATOM_XEMBED_INFO = XInternAtom(mb_kbd_ui_x_display(ui), "_XEMBED_INFO", False);

  list[0] = MAX_SUPPORTED_XEMBED_VERSION;
  list[1] = flags;
  XChangeProperty(mb_kbd_ui_x_display(ui), mb_kbd_ui_x_win(ui), atom_ATOM_XEMBED_INFO, atom_ATOM_XEMBED_INFO, 32,
                  PropModeReplace, (unsigned char*)list, 2);
}

static Bool mb_kbd_xembed_send_message(MBKeyboardUI* ui, Window w, long message, long detail, long data1, long data2) {
  XEvent ev;
  ...
  if (util_untrap_x_errors()) return False;
  ...
  case ClientMessage:
    if (xevent->xclient.message_type == Atom_XEMBED) {
      ...
      case XEMBED_FOCUS_IN:
         ...
         */
        if (ParentEmbedderWin) mb_kbd_xembed_send_message(ui, ParentEmbedderWin, XEMBED_FOCUS_NEXT, 0, 0, 0);
