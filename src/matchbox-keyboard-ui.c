...
static void mb_kbd_ui_resize(MBKeyboardUI* ui, int width, int height);

static int mb_kbd_ui_load_font(MBKeyboardUI* ui);

static char* get_current_window_manager_name(MBKeyboardUI* ui) {
  ...
  XGetWindowProperty(ui->xdpy, RootWindow(ui->xdpy, ui->xscreen), atom_check, 0, 16L, False, XA_WINDOW, &type, &format,
                     &nitems, &bytes_after, (unsigned char**)&support_xwin);

  if (support_xwin == NULL) return NULL;

  atom_utf8_string = XInternAtom(ui->xdpy, "UTF8_STRING", False);
  atom_wm_name = XInternAtom(ui->xdpy, "_NET_WM_NAME", False);

  result = XGetWindowProperty(ui->xdpy, *support_xwin, atom_wm_name, 0, 1000L, False, atom_utf8_string, &type, &format,
                              &nitems, &bytes_after, (unsigned char**)&val);
...
static boolean get_desktop_area(MBKeyboardUI* ui, int* x, int* y, int* width, int* height) {
  Atom atom_area, type;
  int result, format;
  unsigned long nitems, bytes_after;
  int* geometry = NULL;
  ...
  result = XGetWindowProperty(ui->xdpy, RootWindow(ui->xdpy, ui->xscreen), atom_area, 0, 16L, False, XA_CARDINAL, &type,
                              &format, &nitems, &bytes_after, (unsigned char**)&geometry);
...
static void update_display_size(MBKeyboardUI* ui) {
  ...
  if (ui->valid_orientation && ui->dpy_orientation != ui->valid_orientation) mb_kbd_ui_hide(ui);
...
static boolean want_extended(MBKeyboardUI* ui) {
  /* Are we in portrait ? */
  if (ui->dpy_width > ui->dpy_height) return True;

  return False;
}

static boolean get_xevent_timed(Display* dpy, XEvent* event_return, struct timeval* tv) {
...
void mb_kbd_ui_send_press(MBKeyboardUI* ui, const char* utf8_char_in, int modifiers) {
  DBG("Sending '%s'", utf8_char_in);
  fakekey_press(ui->fakekey, (unsigned char*)utf8_char_in, -1, modifiers);
}

void mb_kbd_ui_send_keysym_press(MBKeyboardUI* ui, KeySym ks, int modifiers) {
  fakekey_press_keysym(ui->fakekey, ks, modifiers);
}

void mb_kbd_ui_send_release(MBKeyboardUI* ui) {
  fakekey_release(ui->fakekey);
}

static void mb_kdb_ui_unit_key_size(MBKeyboardUI* ui, int* width, int* height) {
  ...
  *height = 0;
  ...
  while (row_item != NULL) {
    mb_kbd_row_for_each_key(row_item->data, key_item) {
      ...
      if (!mb_kbd_is_extended(ui->kbd) && mb_kbd_key_get_extended(key)) continue;

      /* Ignore keys whose width is forced */
      if (mb_kbd_key_get_req_uwidth(key)) continue;
      ...
      mb_kdb_key_foreach_state(key, state) {
        if (mb_kbd_key_get_face_type(key, state) == MBKeyboardKeyFaceGlyph) {
          ...
          if (mb_kbd_image_width(img) > *width) *width = mb_kbd_image_width(img);

          if (mb_kbd_image_height(img) > *height) *height = mb_kbd_image_height(img);
        }
      }
    }
...
static void mb_kbd_ui_min_key_size(MBKeyboardUI* ui, MBKeyboardKey* key, int* width, int* height) {
  ...
  mb_kdb_key_foreach_state(key, state) {
    if (mb_kbd_key_get_face_type(key, state) == MBKeyboardKeyFaceGlyph) {
      ...
      if (mb_kbd_image_width(img) > max_w) max_w = mb_kbd_image_width(img);

      if (mb_kbd_image_height(img) > max_h) max_h = mb_kbd_image_height(img);
    }
  }
}

static void mb_apply_win_prop(MBKeyboardUI* ui) // Xlab: anti-decorate flags
{

  Atom atom_NET_WM_STATE_SKIP_PAGER, atom_NET_WM_STATE_SKIP_TASKBAR, atom_NET_WM_STATE,
      atom_OB_WM_STATE_UNDECORATED; // Xlab: openbox support

  atom_NET_WM_STATE_SKIP_TASKBAR = XInternAtom(ui->xdpy, "_NET_WM_STATE_SKIP_TASKBAR", False);

  atom_OB_WM_STATE_UNDECORATED = XInternAtom(ui->xdpy, "_OB_WM_STATE_UNDECORATED", False);

  atom_NET_WM_STATE_SKIP_PAGER = XInternAtom(ui->xdpy, "_NET_WM_STATE_SKIP_PAGER", False);

  atom_NET_WM_STATE = XInternAtom(ui->xdpy, "_NET_WM_STATE", False);

  Atom states[] = {atom_NET_WM_STATE_SKIP_TASKBAR, atom_NET_WM_STATE_SKIP_PAGER, atom_OB_WM_STATE_UNDECORATED};

  XChangeProperty(ui->xdpy, ui->xwin, atom_NET_WM_STATE, XA_ATOM, 32, PropModeReplace, (unsigned char*)states, 3);
}

void mb_kbd_ui_allocate_ui_layout(MBKeyboardUI* ui, int* width, int* height) {
  MBKeyboardLayout* layout;
  List *row_item, *key_item;
  int key_y = 0, key_x = 0;
  ...
  while (row_item != NULL) {
    ...
    mb_kbd_row_for_each_key(row, key_item) {
      ...
      if (!mb_kbd_key_get_req_uwidth(key) && key_w < ui->key_uwidth) key_w = ui->key_uwidth;

      if (key_h < ui->key_uheight) key_h = ui->key_uheight;

      key_y = 0;

      if (!mb_kbd_key_is_blank(key)) {
        key_w += 2 * (mb_kbd_keys_border(ui->kbd) + mb_kbd_keys_margin(ui->kbd) + mb_kbd_keys_pad(ui->kbd));
      }

      key_h += 2 * (mb_kbd_keys_border(ui->kbd) + mb_kbd_keys_margin(ui->kbd) + mb_kbd_keys_pad(ui->kbd));

      if (key_h > max_row_key_height) max_row_key_height = key_h;
  ...
  *height = row_y;
  ...
  while (row_item != NULL) {
    ...
    mb_kbd_row_for_each_key(row, key_item) {
      if (!mb_kbd_is_extended(ui->kbd) && mb_kbd_key_get_extended(key_item->data)) continue;

      if (mb_kbd_key_get_fill(key_item->data) || mb_kbd_ui_display_height(ui) <= 320 ||
          mb_kbd_ui_display_width(ui) <= 320)
        n_fillers++;
    }

    if (!n_fillers) goto next_row;
    ...
    mb_kbd_row_for_each_key(row, key_item) {
      ...
      if (mb_kbd_key_get_fill(key_item->data) || mb_kbd_ui_display_height(ui) <= 320 ||
        ...
        for (; nudge_key_item != NULL; nudge_key_item = util_list_next(nudge_key_item)) {
          if (!mb_kbd_is_extended(ui->kbd) && mb_kbd_key_get_extended(nudge_key_item->data)) continue;

          mb_kbd_key_set_geometry(nudge_key_item->data, mb_kbd_key_x(nudge_key_item->data) + (new_w - old_w), -1, -1,
                                  -1);
        }
      }
    }
...
void mb_kbd_ui_redraw_key(MBKeyboardUI* ui, MBKeyboardKey* key) {
  ui->backend->redraw_key(ui, key);
}

static void mb_kbd_ui_redraw_row(MBKeyboardUI* ui, MBKeyboardRow* row) {
  List* key_item;
...
void mb_kbd_ui_swap_buffers(MBKeyboardUI* ui) {
  XClearWindow(ui->xdpy, ui->xwin);
  XSync(ui->xdpy, False);
}

void mb_kbd_ui_redraw(MBKeyboardUI* ui) {
  List* row_item;
  MBKeyboardLayout* layout;
...
void mb_kbd_ui_show(MBKeyboardUI* ui) {
  if (ui->visible) return;

  if (ui->valid_orientation && ui->dpy_orientation != ui->valid_orientation) return;
...
void mb_kbd_ui_hide(MBKeyboardUI* ui) {
  if (!ui->visible) return;
...
void mb_kbd_ui_set_override(MBKeyboardUI* ui, int override) {
  // Xlab: keyboard's absolute positioning
  ui->override = override;
}

void mb_kbd_ui_set_gestures(MBKeyboardUI* ui, int gest) {
  // Xlab: enable gestures
  ui->gest = gest;
}

void mb_kbd_ui_set_geometry(MBKeyboardUI* ui, char* geometry) {
  // Xlab: set geometry
  if (geometry != "") {
    sscanf(geometry, "%dx%d.%d.%d", &ui->imyh, &ui->imyw, &ui->y_shift, &ui->x_shift);
  }
}
void mb_kbd_ui_set_invert(MBKeyboardUI* ui, int invert) {
  // Xlab: place keyboard ontop
  ui->invert = invert;
}

static int mb_kbd_ui_resources_create(MBKeyboardUI* ui) {
...
#define MWM_DECOR_BORDER (1L << 1)
  ...
  Atom /* atom_wm_protocols[3], */
      ...
      atom_NET_WM_WINDOW_TYPE_TOOLBAR, atom_NET_WM_WINDOW_TYPE_DOCK, atom_NET_WM_STRUT_PARTIAL,
      atom_NET_WM_STATE_SKIP_PAGER, atom_NET_WM_STATE_SKIP_TASKBAR, atom_NET_WM_STATE, atom_MOTIF_WM_HINTS,
  ...
  */
  atom_NET_WM_WINDOW_TYPE = XInternAtom(ui->xdpy, "_NET_WM_WINDOW_TYPE", False);

  atom_NET_WM_WINDOW_TYPE_TOOLBAR = XInternAtom(ui->xdpy, "_NET_WM_WINDOW_TYPE_TOOLBAR", False);

  atom_NET_WM_WINDOW_TYPE_DOCK = XInternAtom(ui->xdpy, "_NET_WM_WINDOW_TYPE_DOCK", False);

  atom_MOTIF_WM_HINTS = XInternAtom(ui->xdpy, "_MOTIF_WM_HINTS", False);

  atom_NET_WM_STRUT_PARTIAL = XInternAtom(ui->xdpy, "_NET_WM_STRUT_PARTIAL", False);
  ...
  if (wm_name && streq(wm_name, "matchbox")) have_matchbox_wm = True;

  win_attr.override_redirect = ui->override; /* отвязка */
  win_attr.event_mask = ButtonPressMask | ButtonReleaseMask | Button1MotionMask | StructureNotifyMask;
  ...
  ui->xwin =
      XCreateWindow(ui->xdpy, ui->xwin_root, ui->x_shift, (ui->y_shift != 0) ? ui->y_shift : shift, // Xlab shift's fix
                    ui->xwin_width, ui->xwin_height, 0, CopyFromParent, CopyFromParent, CopyFromParent,
                    CWOverrideRedirect | CWEventMask, &win_attr);

  XSelectInput(ui->xdpy, ui->xwin_root, SubstructureNotifyMask | StructureNotifyMask);
  ...
  XSetStandardProperties(ui->xdpy, ui->xwin, "Keyboard", NULL, 0, NULL, 0, &size_hints);
  ...
  if (!ui->want_embedding) {
    ...
    if (mwm_hints) {
      ...
      XChangeProperty(ui->xdpy, ui->xwin, atom_MOTIF_WM_HINTS, XA_ATOM, 32, PropModeReplace, (unsigned char*)mwm_hints,
    ...
    if (have_ewmh_wm) {
      ...
      int wm_struct_vals[] = {0,
                              /* left */ // Xlab: variants of docking
                              0,         /* right */
                              0,         /* top */
                              0,         /* bottom */
                              0,         /* left_start_y */
      ...
      if (get_desktop_area(ui, NULL, &desk_y, &desk_width, &desk_height)) {
        ...
        XChangeProperty(ui->xdpy, ui->xwin, atom_NET_WM_STRUT_PARTIAL, XA_CARDINAL, 32, PropModeReplace,
                        (unsigned char*)wm_struct_vals, 12);

        DBG("desk width: %i, desk height: %i xwin_height :%i", desk_width, desk_height, ui->xwin_height);
      }

      if (have_matchbox_wm) {
        XChangeProperty(ui->xdpy, ui->xwin, atom_NET_WM_WINDOW_TYPE, XA_ATOM, 32, PropModeReplace,
  ...
  ui->backbuffer =
      XCreatePixmap(ui->xdpy, ui->xwin, ui->xwin_width, ui->xwin_height, DefaultDepth(ui->xdpy, ui->xscreen));
...
ui->imyh=height;
  ...
  if (width_diff < 0 || height_diff < 0) return; /* dont go smaller than our int request - get clipped */
  ...
  width_font_pt_size = ((ui->base_font_pt_size * width) / ui->base_alloc_width);

  if (util_abs(width_font_pt_size - kbd->font_pt_size) > 2) {
    height_font_pt_size = ((ui->base_font_pt_size * height) / ui->base_alloc_height);

    if (util_abs(height_font_pt_size - kbd->font_pt_size) > 2) {
      ui->kbd->font_pt_size = (height_font_pt_size > width_font_pt_size) ? width_font_pt_size : height_font_pt_size;
  ...
  while (row_item != NULL) {
    ...
    mb_kbd_row_for_each_key(row_item->data, key_item) {
      ...
      if (!mb_kbd_is_extended(kbd) && mb_kbd_key_get_extended(key)) continue;

      key_base_width = (mb_kbd_key_width(key) - mb_kbd_key_get_extra_width_pad(key));
      ...
      if (mb_kbd_key_get_fill(key)) n_fillers++;
    }
    ...
    if (n_fillers) {
      ...
      mb_kbd_row_for_each_key(row_item->data, key_item) {
        if (!mb_kbd_is_extended(kbd) && mb_kbd_key_get_extended(key_item->data)) continue;
    ...
    mb_kbd_row_set_x(row_item->data, (width - mb_kbd_row_width(row_item->data)) / 2);

    /* and position down */

    mb_kbd_row_set_y(row_item->data, next_row_y);

    next_row_y += (mb_kbd_row_height(row_item->data) + mb_kbd_row_spacing(ui->kbd));
  ...
  if (next_row_y < height) {
    ...
    while (row_item != NULL) {
      mb_kbd_row_set_y(row_item->data, mb_kbd_row_y(row_item->data) + vspace + 1);
...
void mb_kbd_ui_event_loop(MBKeyboardUI* ui) {
  ...
  while (True) {
    ...
    if (get_xevent_timed(ui->xdpy, &xev, &tvt)) {
      ...
      if (ui->want_embedding) mb_kbd_xembed_process_xevents(ui, &xev);
...
int mb_kbd_ui_display_width(MBKeyboardUI* ui) {
  return ui->dpy_width;
}

int mb_kbd_ui_display_height(MBKeyboardUI* ui) {
  return ui->dpy_height;
}

MBKeyboardUIBackend* mb_kbd_ui_backend(MBKeyboardUI* ui) {
  return ui->backend;
}

Display* mb_kbd_ui_x_display(MBKeyboardUI* ui) {
  return ui->xdpy;
}

int mb_kbd_ui_x_screen(MBKeyboardUI* ui) {
  return ui->xscreen;
}

Window mb_kbd_ui_x_win(MBKeyboardUI* ui) {
  return ui->xwin;
}

Window mb_kbd_ui_x_win_root(MBKeyboardUI* ui) {
  return ui->xwin_root;
}

int mb_kbd_ui_x_win_height(MBKeyboardUI* ui) {
  return ui->xwin_height;
}

int mb_kbd_ui_x_win_width(MBKeyboardUI* ui) {
  return ui->xwin_width;
}

Pixmap mb_kbd_ui_backbuffer(MBKeyboardUI* ui) {
  return ui->backbuffer;
}

MBKeyboard* mb_kbd_ui_kbd(MBKeyboardUI* ui) {
  return ui->kbd;
}

int mb_kbd_ui_realize(MBKeyboardUI* ui) {
  ui->base_font_pt_size = ui->kbd->font_pt_size;

  if (!mb_kbd_ui_load_font(ui)) return 0;

  /* potrait or landscape */
  if (want_extended(ui)) mb_kbd_set_extended(ui->kbd, True);
  ...
  else mb_kbd_xembed_init(ui);

  return 1;
}

int mb_kbd_ui_init(MBKeyboard* kbd) {
  ...
  if ((ui->xdpy = XOpenDisplay(getenv("DISPLAY"))) == NULL) return 0;
...
void mb_kbd_ui_set_embeded(MBKeyboardUI* ui, int embed) {
  ui->want_embedding = embed;
}

int mb_kbd_ui_embeded(MBKeyboardUI* ui) {
  return ui->want_embedding;
}

void mb_kbd_ui_print_window(MBKeyboardUI* ui) {
  fprintf(stdout, "%li\n", mb_kbd_ui_x_win(ui));
  fflush(stdout);
}
...
void mb_kbd_ui_set_daemon(MBKeyboardUI* ui, int value) {
  ui->is_daemon = value;
}

void mb_kbd_ui_limit_orientation(MBKeyboardUI* ui, MBKeyboardDisplayOrientation orientation) {
