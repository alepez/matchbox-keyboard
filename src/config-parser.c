...
struct _keysymlookup {
  KeySym keysym;
  char* name;
} MBKeyboardKeysymLookup[] = {{XK_BackSpace, "backspace"},
                              {XK_Tab, "tab"},
...
} ModLookup[] = {{"shift", MBKeyboardKeyModShift},
...
void set_error(MBKeyboardConfigState* state, char* msg) {
  state->error = True;
  state->error_lineno = XML_GetCurrentLineNumber(state->parser);
  state->error_msg = msg;
}

KeySym config_str_to_keysym(const char* str) {
  int i;

  DBG("checking %s", str);

  for (i = 0; i < sizeof(MBKeyboardKeysymLookup) / sizeof(struct _keysymlookup); i++)
    if (streq(str, MBKeyboardKeysymLookup[i].name)) return MBKeyboardKeysymLookup[i].keysym;
...
MBKeyboardKeyModType config_str_to_modtype(const char* str) {
  int i;

  for (i = 0; i < sizeof(ModLookup) / sizeof(struct _modlookup); i++) {
    DBG("checking '%s' vs '%s'", str, ModLookup[i].name);
    if (streq(str, ModLookup[i].name)) return ModLookup[i].type;
  }

  return 0;
}

static char* get_assets_dir() {
  char* assets_dir = getenv("MB_KBD_ASSETS_DIR");
  return assets_dir ? assets_dir : PKGDATADIR;
}

static char* config_load_file(MBKeyboard* kbd, char* variant_in) {
  ...
  if (getenv("MB_KBD_CONFIG")) {
    ...
    if (util_file_readable(path)) goto load;
  ...
  snprintf(path, 1024, "%s/keyboard%s%s.xml", assets_dir, country == NULL ? "" : country,
  ...
  snprintf(path, 1024, "%s/keyboard%s.xml", assets_dir, variant == NULL ? "" : variant);

  DBG("checking %s\n", path);

  if (util_file_readable(path)) goto load;

  snprintf(path, 1024, "%s/keyboard%s.xml", assets_dir, country == NULL ? "" : country);
  ...
  if (!util_file_readable(path)) return NULL;

load:

  if (stat(path, &stat_info)) return NULL;
...
static const char* attr_get_val(char* key, const char** attr) {
  ...
  while (attr[i] != NULL) {
    if (!strcmp(attr[i], key)) return attr[i + 1];
    i += 2;
  }
...
static void config_handle_key_subtag(MBKeyboardConfigState* state, const char* tag, const char** attr) {
  ...
  if (!strncmp(val, "image:", 6)) {
    ...
    if (val[6] != '/') {
      ...
      if (!util_file_readable(buf)) snprintf(buf, 512, "%s/.matchbox/%s", getenv("HOME"), &val[6]);
    ...
    mb_kbd_key_set_glyph_face(state->current_key, keystate, attr_get_val("display", attr));
  }
  ...
  if ((val = attr_get_val("action", attr)) != NULL) {
    ...
    */
    ...
    if (!strncmp(val, "modifier:", 9)) {
      ...
      if (found_type) {
        mb_kbd_key_set_modifer_action(state->current_key, keystate, found_type);
      } else {
      ...
      if (found_keysym) {
        mb_kbd_key_set_keysym_action(state->current_key, keystate, found_keysym);
      } else {
      ...
      if (strlen(val) > 1 /* match backspace, return etc */
        ...
        mb_kbd_key_set_char_action(state->current_key, keystate, val);
      }
    }
  } else /* fallback to reusing whats displayed  */
  {
    ...
    mb_kbd_key_set_char_action(state->current_key, keystate, attr_get_val("display", attr));
  }
}

static void config_handle_layout_tag(MBKeyboardConfigState* state, const char** attr) {
  const char* val;
...
static void config_handle_row_tag(MBKeyboardConfigState* state, const char** attr) {
  state->current_row = mb_kbd_row_new(state->keyboard);
  mb_kbd_layout_append_row(state->current_layout, state->current_row);
}

static void config_handle_key_tag(MBKeyboardConfigState* state, const char** attr) {
  const char* val;
  DBG("got key");
  ...
  if ((val = attr_get_val("obey-caps", attr)) != NULL) {
    if (strcaseeq(val, "true")) mb_kbd_key_set_obey_caps(state->current_key, True);
  }

  if ((val = attr_get_val("extended", attr)) != NULL) {
    if (strcaseeq(val, "true")) mb_kbd_key_set_extended(state->current_key, True);
  }

  if ((val = attr_get_val("width", attr)) != NULL) {
    if (atoi(val) > 0) mb_kbd_key_set_req_uwidth(state->current_key, atoi(val));
  }

  if ((val = attr_get_val("fill", attr)) != NULL) {
    if (strcaseeq(val, "true")) mb_kbd_key_set_fill(state->current_key, True);
  }
...
static void config_xml_start_cb(void* data, const char* tag, const char** attr) {
  ...
  } else if (streq(tag, "normal") || streq(tag, "default") || streq(tag, "shifted") || streq(tag, "mod1") ||
             streq(tag, "mod2") || streq(tag, "mod3")) {
    config_handle_key_subtag(state, tag, attr);
  }

  if (state->error) {
    fprintf(stderr, "matchbox-keyboard:%s:%d: %s\n", state->keyboard->config_file, state->error_lineno,
            state->error_msg);
    util_fatal_error("Error parsing\n");
  }
}

int mb_kbd_config_load(MBKeyboard* kbd, char* variant) {
  char* data;
  XML_Parser p;
  MBKeyboardConfigState* state;

  if ((data = config_load_file(kbd, variant)) == NULL) util_fatal_error("Couldn't find a keyboard config file\n");

  p = XML_ParserCreate(NULL);

  if (!p) util_fatal_error("Couldn't allocate memory for XML parser\n");
  ...
  if (!XML_Parse(p, data, strlen(data), 1)) {
    fprintf(stderr, "matchbox-keyboard:%s:%d: XML Parse error:%s\n", kbd->config_file, XML_GetCurrentLineNumber(p),
