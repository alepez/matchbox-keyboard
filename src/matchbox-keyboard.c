...
static void mb_kbd_usage(char* progname) {
...
MBKeyboard* mb_kbd_new(int argc, char** argv) {
  ...
  if (variant == NULL) variant = getenv("MB_KBD_VARIANT");

  if (!mb_kbd_ui_init(kb)) return NULL;

  // MBKeyboardUI* ui = kb->ui;

  mb_kbd_ui_set_height_percent(kb->ui, iHeightPercent);

  if (!mb_kbd_config_load(kb, variant)) return NULL;

  kb->selected_layout = (MBKeyboardLayout*)util_list_get_nth_data(kb->layouts, 0);

  if (want_embedding) mb_kbd_ui_set_embeded(kb->ui, True);

  if (override) mb_kbd_ui_set_override(kb->ui, True);

  if (gest) mb_kbd_ui_set_gestures(kb->ui, True);

  if (geometry) mb_kbd_ui_set_geometry(kb->ui, geometry);

  if (invert) mb_kbd_ui_set_invert(kb->ui, True);

  if (want_daemon) {
    mb_kbd_ui_set_daemon(kb->ui, True);
    if (orientation != MBKeyboardDisplayAny) mb_kbd_ui_limit_orientation(kb->ui, orientation);
  }

  if (!mb_kbd_ui_realize(kb->ui)) return NULL;

  if (want_embedding) mb_kbd_ui_print_window(kb->ui);
...
MBKeyboardKeyStateType mb_kbd_keys_current_state(MBKeyboard* kbd) {
  if (mb_kbd_has_state(kbd, MBKeyboardStateShifted)) return MBKeyboardKeyStateShifted;

  if (mb_kbd_has_state(kbd, MBKeyboardStateMod1)) return MBKeyboardKeyStateMod1;

  if (mb_kbd_has_state(kbd, MBKeyboardStateMod2)) return MBKeyboardKeyStateMod2;
...
MBKeyboardKey* mb_kbd_locate_key(MBKeyboard* kb, int x, int y) {
  ...
  while (row_item != NULL) {
    ...
    if (x >= mb_kbd_row_x(row) && x <= mb_kbd_row_x(row) + mb_kbd_row_width(row) && y >= mb_kbd_row_y(row) &&
        y <= mb_kbd_row_y(row) + mb_kbd_row_height(row)) {
      mb_kbd_row_for_each_key(row, key_item) {
        MBKeyboardKey* key = key_item->data;

        if (!mb_kbd_is_extended(kb) && mb_kbd_key_get_extended(key)) continue;

        if (!mb_kbd_key_is_blank(key) && x >= mb_kbd_key_abs_x(key) &&
